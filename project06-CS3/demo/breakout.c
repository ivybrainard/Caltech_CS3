...
const vector_t USER_VEL = {550, 0};

rgb_color_t ball_color = (rgb_color_t){1, .4, .7};
...
void on_key(char key, key_event_type_t type, double held_time, state_t *state) {
  if (type == KEY_PRESSED) {
    ...
    body_set_velocity(state->user, VEC_ZERO);
  }
}
...
void add_ball(state_t *state) {
  ...
  body_t *ball = body_init_with_info(ball_shape, BALL_MASS, ball_color,
...
void add_user(state_t *state) {
  ...
  body_t *user = body_init_with_info(user_shape, INFINITY, user_color,
                                     make_type_info(WALL), free);
  state->user = user;
...
void reset_game(body_t *body1, body_t *body2, vector_t axis, void *aux,
  ...
  for (size_t i = 0; i < scene_bodies(state->scene); i++) {
    ...
    body_type_t *body_type = get_type(body);
    if (body_type == BRICK) {
      body_remove(body);
    }
  }
...
void add_force_creators(state_t *state) {
  for (size_t i = 0; i < scene_bodies(state->scene); i++) {
    ...
    case BRICK:
      ...
      break;
    case WALL:
      create_physics_collision(state->scene, body, state->ball, ELASTICITY);
      break;
    case GROUND:
...
bool emscripten_main(state_t *state) {
  ...
  sdl_render_scene(state->scene, state->user);
}
