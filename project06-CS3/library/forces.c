...
force_t *force_init(force_creator_t func, void *aux, list_t *bodies) {
  force_t *force = malloc(sizeof(force_t));
  assert(force != NULL);
  force->force_func = func;
  force->bodies = bodies;
  force->aux = aux;
  return force;
}
...
void physics_collision_handler(body_t *body1, body_t *body2, vector_t axis,
  ...
  double ua = vec_dot(velocity1, axis);
  double ub = vec_dot(velocity2, axis);
  ...
  double impluse = ((1 + force_const) * (ub - ua)) * reduced_mass;
  vector_t impulse_vec = vec_multiply(impluse, axis);
  ...
  if (mass2 != INFINITY) {
    body_add_impulse(body2, impulse_vec);
  }
}
