...
state_t *emscripten_init() {
  ...
  asset_t *back = asset_make_image(BACKGROUND_PATH, background);
  list_add(state->body_assets, back);

  asset_t *frog_img = asset_make_image_with_body(FROGGER_PATH, froggy);
  state->frog = frog_img;
  list_add(state->body_assets, frog_img);
  ...
  for (size_t r = 3; r < ROWS + 3; r++) {
    ...
    while (cx < MAX.x) {
      ...
      create_collision(state->scene, froggy, obstacle, reset_user_handler, NULL,
                       0);
      asset_t *log_img = asset_make_image_with_body(LOG_PATH, obstacle);
      list_add(state->body_assets, log_img);
    }
  }
