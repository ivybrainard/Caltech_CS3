...
void scene_tick(scene_t *scene, double dt) {
  size_t size = list_size(scene->force_creators);
  for (ssize_t i = 0; i < (ssize_t)size; i++) {
    force_t *f = list_get(scene->force_creators, i);
  ...
  for (ssize_t i = 0; i < (ssize_t)num; i++) {
    ...
    if (body_is_removed(curr_body)) {
      size_t force_length = list_size(scene->force_creators);
      for (ssize_t j = 0; j < (ssize_t)force_length; j++) {
        force_t *curr_force = list_get(scene->force_creators, j);
...
scene_t *scene_init(void) {
  ...
  scene->force_creators = list_init(1, (free_func_t)force_free);
  return scene;
}
