...
list_t *generate_memes(state_t *state) {
  ...
  for (size_t i = 0; i < NUM_MEMES; i++) {
    ...
    SDL_Rect bg = {meme.bg_loc.x, meme.bg_loc.y, meme.bg_size.x,
                   meme.bg_size.y};
    ...
    for (size_t j = 0; j < meme.num_captions; j++) {
      SDL_Rect text_rect = {meme.locations[j].x, meme.locations[j].y, bg.w,
                            bg.h};
      asset_t *text_asset =
          asset_make_text(meme.font_path, text_rect, meme.captions[j], BLUE);
