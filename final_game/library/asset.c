...
asset_t *asset_make_image(const char *filepath, SDL_Rect bounding_box) {
  ...
  image_asset->texture =
      (SDL_Texture *)asset_cache_obj_get_or_create(ASSET_IMAGE, filepath, 0);
...
asset_t *asset_make_image_with_body(const char *filepath, body_t *body) {
  ...
  image_asset_t *image_asset =
      (image_asset_t *)asset_init(ASSET_IMAGE, (get_bound_box(body)));
...
asset_t *asset_make_text(const char *filepath, SDL_Rect bounding_box,
  ...
  text_asset->font =
      asset_cache_obj_get_or_create(ASSET_FONT, filepath, TEXT_SIZE);
...
asset_t *asset_make_button(SDL_Rect bounding_box, asset_t *image_asset,
  ...
  asset_cache_register_button((asset_t *)button_asset);
...
void asset_render(asset_t *asset) {
  ...
  if (asset->type == ASSET_IMAGE) {
    ...
    if (image_asset->body != NULL) {
      asset->bounding_box = get_bound_box(image_asset->body);
    }
    ...
    vector_t location = {asset->bounding_box.x, asset->bounding_box.y};
    render_text(text_asset->font, text_asset->text, location);
